...
// Minimal scrollbar + focus styles. Injected once on the client so SSR doesn't
// touch `document`.
...
`;
const STYLE_FLAG = "data-ai-prompt-box-styles";
function useInjectStyles() {
  React.useEffect(() => {
    if (typeof document === "undefined") return;
    if (document.querySelector(`style[${STYLE_FLAG}]`)) return;
    const el = document.createElement("style");
    el.setAttribute(STYLE_FLAG, "");
    el.textContent = PROMPT_BOX_STYLES;
...
type Bar = { height: number; delay: number; duration: number };
...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  // Keep callback refs current without re-triggering the recording effect.
  React.useEffect(() => {
    onStartRef.current = onStartRecording;
    onStopRef.current = onStopRecording;
  });

  // Visualizer bars use Math.random — generate on mount only so SSR/CSR
  // don't mismatch (the bars are invisible on the server anyway).
  React.useEffect(() => {
    setBars(
